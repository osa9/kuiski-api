...
class LambdaApi {
    ...
    getParameters(paramDefinitions) {
        ...
        Object.keys(paramDefinitions).forEach((defKey) => {
            ...
            if(!(def.name in target) && def.required) {
                errors.push(`Parameter Error: Mandatory parameter ${def.name} is missing`);
                return null;
            }
            ...
            if(value === undefined) {
                errors.push(`Validation Error: Parameter "${def.name}" does not match type "${def.type}"`);
            }
    ...
    execute(handler) {
        new Promise((resolve) => {
            ...
            const res = handler(new ApiRequestHandler(params, this.event, this.context));  
            resolve(res);
        }).then((res) => {
